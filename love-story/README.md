...
**không đụng gì tới backend**. GitHub Pages chỉ phục vụ file tĩnh nên không
thể tự chạy Node.js/MongoDB — backend phải host riêng, chạy 24/7 ở nơi khác.

Hướng dẫn chạy local + deploy backend (Render + MongoDB Atlas, cả 2 free)
đầy đủ nằm ở **[`../backend/README.md`](../backend/README.md)**. Phần FE
chỉ cần biết:

- **Dev local**: mặc định FE gọi API tới `http://localhost:3003` (xem
  `src/utils/api.js`) — chạy backend local đúng port đó là trang Login hoạt
  động ngay, không cần cấu hình gì thêm. Tài khoản test có sẵn: `love` / `123456`.
- **Deploy production**: sau khi backend đã chạy ở 1 URL public (vd
  `https://lovestory-backend.onrender.com`), set URL đó vào
  `love-story/.env.production`:
  ```
  VITE_API_URL=https://<url-backend-that>
  ```
  Vite tự nạp file này khi `npm run build`/`npm run deploy`. Backend cũng
  cần set `CORS_ORIGINS=https://<username>.github.io` (không để `*`) để
  trình duyệt cho phép gọi cross-origin.

Nếu chưa deploy backend, trang vẫn lên GitHub Pages bình thường — chỉ riêng
nút đăng nhập báo lỗi "Không thể kết nối máy chủ" tới khi `VITE_API_URL`
trỏ đúng backend đang chạy.
